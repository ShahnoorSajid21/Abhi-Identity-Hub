...
### Found by the end-to-end review, 23 August 2026

| ID | Gap | Severity | Action |
|---|---|---|---|
| **NEW-08** | **Log redaction missed the CNIC format that actually arrives** — `scrubString` matched `\d{13,}` only, so `61101-1234567-8` passed through unmasked. `GET /kyc/history?cnic=` and `GET /audit/events?cnic=` take the CNIC in a query string and the request logger writes `path: req.url` verbatim, so one customer lookup wrote a citizen's primary identifier to stdout | **High** | **FIXED.** Separator-aware 5-7-1 rule added; 64-hex identifiers lifted out so they survive. Pinned by `SEC-15` in `tests/security/regressions.test.ts` |
| **NEW-09** | **SEC-05 roster gate could not engage over HTTP** — `POST /employer/bulk-lookup` called the service with no `employerId`, and the bootstrap constructed no `EmploymentRegister`. With a register configured every request failed; without one — the shipped configuration — the endpoint was an unrestricted existence oracle over the customer base. The control was built, unit-tested and reported `IMPLEMENTED`; it was never connected | **High** | **FIXED.** Employer identity now taken from the authenticated principal (`OU=employer:<id>` in the client certificate, `x-abhi-employer` on the dev shim), register wired and seeded in `server.ts`, and the gateway refuses to construct without one when `NODE_ENV=production`. Pinned by three HTTP-level tests |
| **NEW-10** | **Per-subject rate limiting did not cover the `subjectId` path** — the enumeration and cost-attack control keyed on `cnic` alone, but `/kyc/verify` accepts and prefers `subjectId`, the console sends nothing else, and the customer read routes are `/customers/{subjectId}/…`. Enumeration by subject id, the form an attacker would use, was unlimited | **High** | **FIXED.** All three identifier positions are now counted |
| **NEW-11** | **The e-CIB result was discarded** — the check was awaited on every non-DENY origination and its return value dropped, so an adverse credit record produced a response byte-identical to a clean one. The mock always answered clean, so no test could catch it, and dropping a real provider in behind it would have preserved the behaviour exactly | **High** | **FIXED.** Outcome carried out as `VerifyResult.eCib` and rendered in the console. Deliberately still NOT an input to `decide()` — credit standing is not identity — see `ECibOutcome` |
| **NEW-12** | **[OPEN-E] Identity assurance and transaction authorisation are not distinguished** — Product Manual Part Two §9.3 puts fingerprint *and* live-selfie on EWA, ASA and SBL alike. If that is CDD, EWA/ASA should be A3 and are configured A2. If it is per-request transaction authorisation — which §9.3's own wording suggests — then it is not reusable by anything, and part of what the demo counts as avoided cost is spend that occurs regardless | **High (business case)** | **OPEN — Compliance + product decision.** Not silently changed: either reading moves the numbers the programme is being sold on. Documented in `packages/policy/src/policies.ts` and carried as a caveat on the savings figures |
| **NEW-13** | **The console was outside every gate** — `npm run typecheck` covers the root tsconfig only, `npm test` does not invoke vitest, and no CI job ran either. 55 console tests ran only on a developer's machine, and a type error in `apps/web` reached main with every build green — the exact failure the typecheck gate's own comment warns about | Medium | **FIXED.** `typecheck:web` added, `verify` extended, and a `console` CI gate added running vitest plus the production build |
