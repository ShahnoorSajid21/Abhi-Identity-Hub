 ...
 *
 * IMPORTANT — these are ENGINEERING DEFAULTS drawn from ABHI's internal
 * product documentation. They are NOT Compliance-approved policy. Sign-off by
 ...
 * liveness. ABHI's product documentation says otherwise: it puts a fingerprint
 * and a live-selfie face check across EWA, ASA and SBL alike, documented once
 * for all three rather than per product.
 *
 ...
 * from that documentation alone. The requirement sits in the employee-app
 * chapter and describes a per-REQUEST journey — a biometric step between
 * reviewing the details and the request reaching approval, under a daily
 * attempt cap. That reads as transaction authorisation — proving the person
 * asking for this money is
 ...
 * If that biometric is transaction authorisation, then some of what the
 ...
 * without that decision is stop the ambiguity being invisible: see [OPEN-F]
 * in the gap analysis and the caveat carried on the savings figures.
 * ---------------------------------------------------------------------------
 */
...
export const PRODUCT_POLICIES: Readonly<Record<string, ProductPolicy>> = Object.freeze({
  // [OPEN-F] applies to EWA and ASA: ABHI's product documentation puts a
